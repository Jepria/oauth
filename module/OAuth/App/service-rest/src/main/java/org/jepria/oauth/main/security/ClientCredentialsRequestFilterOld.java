...
public final class ClientCredentialsRequestFilterOld implements ContainerRequestFilter {
    ...
    if (authString != null) {
      ...
      try (InputStream in = requestContext.getEntityStream()) {
        ...
        if (parameters.get(CLIENT_ID) != null && parameters.get(CLIENT_SECRET) != null) {
          ...
          requestContext.setSecurityContext(new ClientSecurityContext(clientId, parameters.get(CLIENT_ID)));
        } else if (parameters.get(CLIENT_ID) != null && parameters.get(CLIENT_SECRET) == null) {
